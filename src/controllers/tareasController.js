...
exports.listarTareas = (req, res) => {
    const { estado, prioridad, fecha } = req.query; // Obtener los filtros de la consulta
    let tareas = Tarea.getAll();
    const estados = Estado.getAll();
    const prioridades = Prioridad.getAll();
    const usuarios = Usuario.getAll();
    ...
    if (estado) {
        tareas = tareas.filter(tarea => tarea.estado === parseInt(estado));
    }

    if (prioridad) {
        tareas = tareas.filter(tarea => tarea.prioridad === parseInt(prioridad));
    }

    if (fecha) {
        tareas = tareas.filter(tarea => tarea.fechaVencimiento === fecha);
    }
    ...
    res.render('tareas/listar', { 
        ...
        prioridades, 
        usuarios, 
        filtros: { estado, prioridad, fecha }, // Pasar los filtros a la vista
